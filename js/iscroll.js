...
function iScroll (el, options) {
	...
	that.options = {
		...
		snap: false,
		bounceLock: false,
		scrollbarColorH: 'rgba(0,0,0,0.5)',
		scrollbarColorV: 'rgba(0,0,0,0.5)',
...
iScroll.prototype = {
	...
	refresh: function () {
		...
		that.scrollWidth = 460+80;
		that.scrollHeight = that.wrapper.clientHeight;
		that.scrollHeight = 12;
		...
		if (that.options.hScrollbar && that.scrollX) {
			that.scrollBarX = that.scrollBarX || new scrollbar('horizontal', that.wrapper, that.options.fadeScrollbar, that.options.shrinkScrollbar, that.options.scrollbarColorH);
		...
		if (that.options.vScrollbar && that.scrollY && that.scrollerHeight > that.scrollHeight) {
			that.scrollBarY = that.scrollBarY || new scrollbar('vertical', that.wrapper, that.options.fadeScrollbar, that.options.shrinkScrollbar, that.options.scrollbarColorV);
...
//			that.dist+= Math.abs(leftDelta) + Math.abs(topDelta);
	...
	scrollToPage: function (pageX, pageY, runtime) {
		...
		if (pageX == 'next') {
			...
			pageX = $('section#container article').length-1;
		} else if (pageX == 'prev') {
			pageX = --that.pageX;
		}
